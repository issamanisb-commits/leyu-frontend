...
import {
  ...
  useSingleSuperAdminDatasetLanguage,
...
export default function UsersPage() {
  ...
  } = useSingleSuperAdminDatasetLanguage(viewTypeDatasetLanguage);
  const projecIcon = (
    <svg
      width="47"
      height="48"
  ...
  // Metrics data using superadmin statistics
  const metrics = superadminData?.data
    ? [
        {
          title: "Total Projects",
          value: superadminData.data.total_projects,
          change: "bg-white",
          icon: projecIcon,
        },
        {
        ...
        },
        {
          ...
          value:
            ...
            superadminData.data.total_project_managers,
          change: "bg-blue-100",
          icon: totalUsersIcon,
        },
      ]
  ...
  // Data for Language/Dialect Distribution (Bar Chart with Gradient)
  const barData: ChartData<"bar", number[], string> = {
    labels: datasetDatanDatasetLanguage?.data
      ? datasetDatanDatasetLanguage.data.map((item: DatasetLanguageItem) =>
          viewTypeDatasetLanguage === "LANGUAGE"
            ? item.language_name || "Unknown Language"
            : item.dialect_name || "Unknown Dialect"
        )
      : ["No Data"],
    datasets: [
      {
        label:
          viewTypeDatasetLanguage === "LANGUAGE" ? "Languages" : "Dialects",
        data: datasetDatanDatasetLanguage?.data
          ? datasetDatanDatasetLanguage.data.map(
              (item: DatasetLanguageItem) => parseInt(item.count) || 0
            )
          : [0],
        backgroundColor: (context: ScriptableContext<"bar">): Color => {
          const chart = context.chart;
          const { ctx, chartArea } = chart;
          if (!chartArea) return "rgba(59, 130, 246, 0.6)" as Color;
          ...
          gradient.addColorStop(0, "rgba(10,102,173,255)");
          gradient.addColorStop(1, "rgba(52,162,171,255)");
          return gradient as CanvasGradient as Color;
        },
  ...
  // Compute labels and data separately to avoid initialization issues
  ...
  // Toggle handler for LANGUAGE/DIALECT switch
  const handleToggleLanguageDialect = () => {
    setViewTypeDatasetLanguage(
      viewTypeDatasetLanguage === "LANGUAGE" ? "DIALECT" : "LANGUAGE"
    );
  };
        ...
        <div className="w-full min-h-screen bg-white p-2">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-1 lg:grid-cols-4 gap-4 mb-6">
            {metrics.map((metric, index) => (
              <div
                key={index}
                className={`${metric.change} px-3 py-5 flex flex-row rounded-lg shadow-sm items-center gap-2`} // Applied px-3 and py-5 to the card
              >
                <div>{metric.icon}</div>
                <div className="py-2 px-1">
                  {" "}
                  {/* Kept reduced padding for text container */}
            ...
            <div className="bg-white p-4 rounded-lg ">
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-lg font-semibold text-gray-900">
                  Language and Dialect Distribution
                </h2>
              </div>
              <div className="h-70">
                <div className="flex justify-between items-center mb-4">
                  ...
                  <button
                    className="relative inline-flex items-center h-10 rounded-full w-51 bg-gray-300 focus:outline-none"
                    onClick={handleToggleLanguageDialect}
                    aria-label={`Toggle between Language and Dialect, currently ${viewTypeDatasetLanguage}`}
                  >
                    <span
                      className={`inline-flex h-9 w-24.5 ml-1  items-center justify-center rounded text-sm ${
                        viewTypeDatasetLanguage === "LANGUAGE"
                    ...
                    <span
                      className={`inline-flex h-9 w-24.5 items-center justify-center rounded text-sm ${
                        viewTypeDatasetLanguage === "DIALECT"
                          ? "bg-white text-blue-500 rounded-2xl "
                          : "bg-gray-300 text-black rounded-2xl"
                      }`}
                    >
                ...
                <Bar className="flex" data={barData} options={barOptions} />
              </div>
            </div>
